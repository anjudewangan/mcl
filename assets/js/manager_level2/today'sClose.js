...
function pagination() {
  let api = "";
  if (searchParam) {
    api = `${search_api}?status=Close&todayClose=true&search=${searchParam}&area=${area}&category=${category}`;
  } else {
    api = `${pagination_api}?status=Close&todayClose=true&area=${area}&category=${category}`;
  }
