...
fetch(`${baseUrl}/colony`)
  ...
  .then((data) => {
    let optionList = ``;
    data.map((value) => {
      optionList += ` <option>${value.Area}</option>`;
    });
    document.getElementById("areaSelection").innerHTML += optionList;
  });
...
function area_category_search() {
  area = document.getElementById("areaSelection").value;
  category = document.getElementById("categorySelection").value;
  progressStatus = document.getElementById("progressData").value;
  pagination();
}
      ...
      fetch(path2show, {
        ...
        .then((res) => {
          ...
          fetch(addTech_api)
            ...
            .then((data) => {
              data.map((values, _) => {
                console.log(values.Name, callerTechnician);
                if (
