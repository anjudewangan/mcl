...
function simpleTemplating(data, page) {
  ...
  data[0].Complains.forEach((value) => {
    ...
    console.log(value.Inprogress.slice(-1)[0]);
    html += `
          <tr>
          <td>${count}</td>
      ...
      complainTime === "Invalid Date" ? "" : complainTime
         ...
         ${
           value.Inprogress[value.Inprogress.length - 1] === "Close"
             ? ""
             : `<td><button type="button"
                class="btn btn-success-gradien btn-sm btn-update"
                data-bs-toggle="modal" key=${value._id}>
                Force Close
            </button></td>`
         }
     </tr>
...
function pagination() {
  let api = "";
  if (progressStatus) {
    api = `${escalated_complains_api}?role=caller&time=timel2&status=${progressStatus}${
      startDate ? `&startDate=${startDate}&endDate=${endDate}` : ""
    }`;
  } else if (searchParam) {
    api = `${escalated_complains_api}?role=caller&time=timel2&search=${searchParam}`;
  } else {
    api = `${escalated_complains_api}?role=caller&time=timel2${
      startDate ? `&startDate=${startDate}&endDate=${endDate}` : ""
    }`;
  }
            ...
            .then((data) => {
              data.map((values, _) => {
                if (
                  ...
                  values.Location.match(callerColony)
                ) {
...
function exportData() {
  let api = "";
  if (progressStatus) {
    api = `/export/escalated?role=caller&time=timel2&status=${progressStatus}${
      startDate ? `&startDate=${startDate}&endDate=${endDate}` : ""
    }`;
  } else if (searchParam) {
    api = `/export/escalated?role=caller&time=timel2&search=${searchParam}`;
  } else {
    api = `/export/escalated?role=caller&time=timel2${
      startDate ? `&startDate=${startDate}&endDate=${endDate}` : ""
    }`;
  }
