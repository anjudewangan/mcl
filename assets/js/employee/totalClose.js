// console.log("complain close List");
...
async function renderTable(page = 1) {
  ...
  let api = `${pagination_api}?status=Close&page=${page}&eis=${eis}`;
  if (searchParam) {
    api = `${search_api}?status=Close&search=${searchParam}&page=${page}&eis=${eis}`;
  }
  fetch(api)
    .then((res) => {
      if (res.status === 402) {
    ...
    .then((data) => {
      ...
      data[0].Complains.map((value, index) => {
        ...
        <tr>
        <td>${count}</td>
        <td class=' position-sticky start-0 userList'><a key=${
          value._id
        } class="text-primary cursor-pointer"  data-bs-toggle="modal"
        >${value.Complain_Number}</a></td>
...
//Fetch Data from API
function getData() {
  let api = `${pagination_api}?status=Close&eis=${eis}`;
  if (searchParam) {
    api = `${search_api}?status=Close&search=${searchParam}&eis=${eis}`;
  }
