...
function pagination() {
  let api = "";
  if (progressStatus) {
    api = `${escalated_complains_api}?role=managerl2&time=timel2&status=${progressStatus}&area=${area}&category=${category}${
    ...
    api = `${escalated_complains_api}?role=managerl2&time=timel2&area=${area}&category=${category}${
      startDate ? `&startDate=${startDate}&endDate=${endDate}` : ""
    }`;
  }
