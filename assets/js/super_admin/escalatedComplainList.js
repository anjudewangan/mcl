...
function pagination() {
  let api = "";
  if (progressStatus) {
    api = `${escalated_complains_api}?role=superAdmin&time=timel2&status=${progressStatus}${startDate ? `&startDate=${startDate}&endDate=${endDate}` : ""
      }`;
  } else if (searchParam) {
    api = `${escalated_complains_api}?role=superAdmin&time=timel2&search=${searchParam}`;
  } else {
    api = `${escalated_complains_api}?role=superAdmin&time=timel2${startDate ? `&startDate=${startDate}&endDate=${endDate}` : ""
      }`;
    console.log("object");
  }
